module user_frng import croc_pkg::*; (
 ...
 // ============================================================================
 // 1.1 USER SEED REGISTERS AND CONTROL
 // ============================================================================
 logic [31:0] user_seed_0_q, user_seed_1_q, user_seed_2_q, user_seed_3_q;
 logic [31:0] user_seed_ctrl_q;
 ...
 // ============================================================================
 // 2.1 USER SEED ADDRESSES
 // ============================================================================
 localparam int USER_SEED_0_ADDR   = 4'h8; // e.g., offset 0x20
 localparam int USER_SEED_1_ADDR   = 4'h9;
 localparam int USER_SEED_2_ADDR   = 4'hA;
 localparam int USER_SEED_3_ADDR   = 4'hB;
 localparam int USER_SEED_CTRL_ADDR= 4'hC;
 ...
 always_comb begin
   ...
   case (seed_st_q)
     ...
     DONE: begin
       // If user requests reseed, go back to IDLE to restart seeding
       if (user_seed_ctrl_q == 32'h1)
         seed_st_d = IDLE;
       else
         seed_st_d = DONE;
     end
 ...
 // ============================================================================
 logic use_user_seed;
 assign use_user_seed = (user_seed_ctrl_q == 32'h1); // or any other trigger value
 ...
 always_ff @(posedge clk_i or negedge rst_ni) begin
   if (!rst_ni) begin
     ...
     if (seed_st_q == WAIT) begin
       if (use_user_seed) begin
         // Use user-provided seed words
         unique case (seed_cnt_q)
           2'd0: status_0_q <= user_seed_0_q;
           2'd1: status_1_q <= user_seed_1_q;
           2'd2: status_2_q <= user_seed_2_q;
           2'd3: status_3_q <= user_seed_3_q;
         endcase
       end else begin
         // Default: use SRAM-based seeding
         unique case (seed_cnt_q)
       ...
       // Reset user_seed_ctrl_q if reseed was requested
       if (user_seed_ctrl_q == 32'h1)
         user_seed_ctrl_q <= 32'd0;
     end
     ...
     if (obi_req_i.req && obi_req_i.a.we) begin
       case (obi_req_i.a.addr[4:2])
         USER_SEED_0_ADDR:    user_seed_0_q    <= obi_req_i.a.wdata;
         USER_SEED_1_ADDR:    user_seed_1_q    <= obi_req_i.a.wdata;
         USER_SEED_2_ADDR:    user_seed_2_q    <= obi_req_i.a.wdata;
         USER_SEED_3_ADDR:    user_seed_3_q    <= obi_req_i.a.wdata;
         USER_SEED_CTRL_ADDR: user_seed_ctrl_q <= obi_req_i.a.wdata;
         default: ; // no action
       endcase
     end
   end
